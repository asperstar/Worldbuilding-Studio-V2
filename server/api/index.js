...
let grokModel;
try {
  grokModel = createXai({ apiKey: process.env.XAI_API_KEY })('grok-3');
  console.log('Grok model initialized successfully');
} catch (error) {
  console.error('Failed to initialize Grok model:', error);
  process.exit(1);
}
  ...
  corsMiddleware(req, res, async () => {
    ...
    if (req.method === 'POST' && req.url === '/chat') {
      try {
        ...
        res.status(200).json({ response: grokResponse.text });
      } catch (error) {
...
Describe the map's layout, key features, connections between environments, and visual style (e.g., dark forest, medieval village). Format as a single paragraph, max 200 words.`;
        ...
        res.status(200).json({ description: grokResponse.text });
      } catch (error) {
    ...
    if (req.method === 'POST' && req.url === '/generate-map') {
      try {
        ...
        // If no prompt provided, generate one from environments
        if (!mapPrompt) {
          if (!environments || !Array.isArray(environments)) {
